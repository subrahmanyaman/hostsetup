...
keymaster_error_t translateRkpErrorCode(keymaster_error_t error) {
    switch(static_cast<int32_t>(-error)) {
        ...
        case kStatusInvalidState:
            return static_cast<keymaster_error_t>(-IRemotelyProvisionedComponent::STATUS_FAILED);
        case kStatusInvalidMac:
            return static_cast<keymaster_error_t>(IRemotelyProvisionedComponent::STATUS_INVALID_MAC);
        case kStatusProductionKeyInTestRequest:
            return static_cast<keymaster_error_t>(IRemotelyProvisionedComponent::STATUS_PRODUCTION_KEY_IN_TEST_REQUEST);
        case kStatusTestKeyInProductionRequest:
            return static_cast<keymaster_error_t>(IRemotelyProvisionedComponent::STATUS_TEST_KEY_IN_PRODUCTION_REQUEST);
        case kStatusInvalidEek:
            return static_cast<keymaster_error_t>(IRemotelyProvisionedComponent::STATUS_INVALID_EEK);
    }
    return error;
}
...
JavacardRemotelyProvisionedComponentDevice::getHardwareInfo(RpcHardwareInfo* info) {
  //LOG(ERROR) << "venkat JavacardRemotelyProvisionedComponentDevice::getHardwareInfo step1";
...
JavacardRemotelyProvisionedComponentDevice::generateEcdsaP256KeyPair(bool testMode,
    ...
    if (err != KM_ERROR_OK) {
        ...
        return kmError2ScopedAStatus(translateRkpErrorCode(err));
    }
...
JavacardRemotelyProvisionedComponentDevice::generateCertificateRequest(bool testMode,
    ...
    uint32_t respFlag;
    std::cout << "venkat beginSendData" << std::endl;
    auto ret = beginSendData(testMode, keysToSign);
    if (!ret.isOk()) return ret;

    std::cout << "venkat updateMacedKey" << std::endl;
    ret = updateMacedKey(keysToSign);
    if (!ret.isOk()) return ret;

    std::cout << "venkat updateChallenge" << std::endl;
    ret = updateChallenge(challenge);
    if (!ret.isOk()) return ret;

    std::cout << "venkat updateEEK" << std::endl;
    ret = updateEEK(endpointEncCertChain);
    if (!ret.isOk()) return ret;

    std::cout << "venkat finishSendData" << std::endl;
    ...
    std::cout << "venkat getResponse" << std::endl;
        ret = getResponse(cipheredData, recipients, respFlag);
        if (!ret.isOk()) return ret;
    }
    std::cout << "venkat deviceInfo len: " << deviceInfo->deviceInfo.size() << " keysToSignMac len: " << keysToSignMac->size() << std::endl;
    ...
    protectedData->protectedData =
        cppbor::Array()
            ...
            .encode();
    std::cout << "venkat generateCertificateRequest successful." << std::endl;
    return ScopedAStatus::ok();
}
