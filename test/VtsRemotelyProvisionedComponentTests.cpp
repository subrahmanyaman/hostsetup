...
PrintInstanceNameToString(const testing::TestParamInfo<std::string>& info) {
...
inline bool operator==(const AuthorizationSet& a, const AuthorizationSet& b) {
...
class VtsRemotelyProvisionedComponentTests : public testing::TestWithParam<std::string> {
  public:
    virtual void SetUp() override {
           std::shared_ptr<SocketTransport> mSocketTransport = std::make_shared<SocketTransport>();
           card =
            std::make_shared<JavacardSecureElement>(mSocketTransport, getOsVersion(),
                                                getOsPatchlevel(), getVendorPatchlevel());
        provisionable_ = std::make_shared<JavacardRemotelyProvisionedComponentDevice>(card);
        //TODO add below commented statement.
        //ASSERT_TRUE(provisionable_->getHardwareInfo(&rpcHardwareInfo).isOk());
        provisionable_->getHardwareInfo(&rpcHardwareInfo);
        std::cout << "RPC HardwareInfo: " << rpcHardwareInfo.supportedEekCurve;
...
/*        if (AServiceManager_isDeclared(GetParam().c_str())) {
    ...
    static vector<string> build_params() {
        return {"keymaster.javacard.rkp.test-host"};
...
TEST_P(GenerateKeyTests, generateAndUseEcdsaP256Key_prodMode) {
    ...
    std::shared_ptr<IKeyMintDevice> keyMint = std::make_shared<JavacardKeyMintDevice>(card);
...
TEST_P(CertificateRequestTest, NonEmptyRequestCorruptMac_testMode) {
    ...
    EXPECT_EQ(status.getServiceSpecificError(), IRemotelyProvisionedComponent::STATUS_INVALID_MAC);
}
/**
...
TEST_P(CertificateRequestTest, NonEmptyCorruptEekRequest_prodMode) {
    ...
    for (int ii = 0; ii < parsedChain->asArray()->size(); ++ii) {
        ...
        ASSERT_EQ(status.getServiceSpecificError(),
                  IRemotelyProvisionedComponent::STATUS_INVALID_EEK);
    }
}
...
TEST_P(CertificateRequestTest, NonEmptyIncompleteEekRequest_prodMode) {
    ...
    auto status = provisionable_->generateCertificateRequest(
            ...
            &keysToSignMac);
    ASSERT_FALSE(status.isOk());
    ASSERT_EQ(status.getServiceSpecificError(), IRemotelyProvisionedComponent::STATUS_INVALID_EEK);
}
/**
...
TEST_P(CertificateRequestTest, NonEmptyRequest_prodKeyInTestCert) {
    ...
    ASSERT_FALSE(status.isOk());
    ASSERT_EQ(status.getServiceSpecificError(),
              IRemotelyProvisionedComponent::STATUS_PRODUCTION_KEY_IN_TEST_REQUEST);
}
/**
...
TEST_P(CertificateRequestTest, NonEmptyRequest_testKeyInProdCert) {
    ...
    ASSERT_FALSE(status.isOk());
    ASSERT_EQ(status.getServiceSpecificError(),
              IRemotelyProvisionedComponent::STATUS_TEST_KEY_IN_PRODUCTION_REQUEST);
}
#endif
