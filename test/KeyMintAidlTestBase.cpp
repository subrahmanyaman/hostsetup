...
AuthorizationSet filtered_tags(const AuthorizationSet& set) {
    ...
    std::remove_copy_if(set.begin(), set.end(), std::back_inserter(filtered),
                        [](const auto& entry) -> bool {
                            return std::find(kTagsToFilter.begin(), kTagsToFilter.end(),
                                             entry.tag) != kTagsToFilter.end();
                        });
    return filtered;
}
...
void KeyMintAidlTestBase::SetUp() {
    ...
    InitializeKeyMint(std::make_shared<JavacardKeyMintDevice>(std::make_shared<JavacardSecureElement>(
        std::make_shared<SocketTransport>(), getOsVersion(), getOsPatchlevel(), getVendorPatchlevel()
    )));
}
...
ErrorCode KeyMintAidlTestBase::ImportWrappedKey(string wrapped_key, string wrapping_key,
    ...
    Status result = keymint_->importWrappedKey(
        ...
        vector<uint8_t>(masking_key.begin(), masking_key.end()), unwrapping_params.vector_data(),
        0 /* passwordSid */, 0 /* biometricSid */, &creationResult);
...
std::pair<ErrorCode, vector<uint8_t>>
KeyMintAidlTestBase::UpgradeKey(const vector<uint8_t>& key_blob) {
...
vector<Digest> KeyMintAidlTestBase::ValidDigests(bool withNone, bool withMD5) {
    ...
    case SecurityLevel::TRUSTED_ENVIRONMENT:
        if (withNone) {
            if (withMD5)
                return {Digest::NONE,      Digest::MD5,       Digest::SHA1,     Digest::SHA_2_224,
...
const vector<KeyParameter>&
KeyMintAidlTestBase::SecLevelAuthorizations(const vector<KeyCharacteristics>& key_characteristics) {
...
KeyMintAidlTestBase::SecLevelAuthorizations(const vector<KeyCharacteristics>& key_characteristics,
    ...
    auto found =
        std::find_if(key_characteristics.begin(), key_characteristics.end(),
...
ErrorCode KeyMintAidlTestBase::UseRsaKey(const vector<uint8_t>& rsaKeyBlob) {
    ...
    auto [result, signature] =
        ProcessMessage(rsaKeyBlob, KeyPurpose::SIGN, message,
...
bool verify_attestation_record(const string& challenge,                //
    ...
    if (!strcmp(property_value, "green")) {
        ...
        EXPECT_NE(
            0, memcmp(verified_boot_key.data(), empty_boot_key.data(), verified_boot_key.size()));
...
void check_maced_pubkey(const MacedPublicKey& macedPubKey, bool testMode,
    ...
    auto macFunction = [](const cppcose::bytevec& input) {
        return cppcose::generateHmacSha256(kTestMacKey, input);
    };
    auto testTag =
